...
- **Anomaly Detection:**
  - MCP `detect_patterns` tool: cross-cutting early warning system that surfaces unusual patterns across the entire project. Scans 11 data sources in parallel and checks for: velocity drops (7d vs 30d comparison), backlog growth (creation outpacing closure), rework trends (rate and worsening trajectory), DORA change failure rate, process violations (stale items, review bottleneck, low flow efficiency), dependency issues (cycles, critical bottlenecks, orphaned blocked), capacity risks (decelerating contributors, bus factor 1 areas), and knowledge decay (critical risk files, decay alerts). Each anomaly includes category, severity, trend direction (worsening/new/stable/improving), evidence, affected issues, and suggested action. Returns a health snapshot and natural-language summary. The "things you should know about" early warning system.
- `validate.sh`: Added check for anomaly.ts
- **Smart Triage:**
  - MCP `triage_issue` tool: one-call complete issue assessment that auto-classifies tier (1/2), type (bug/feature/spike/epic/chore), area (frontend/backend/contracts/infra), and area confidence. Recommends priority using factor analysis (urgency/impact/dependencies/effort) with transparent reasoning. Estimates size from completion prediction. Assesses risk from rework probability and completion risk signals. Finds similar past work via `suggest_approach`. Suggests assignment from team capacity area coverage. Identifies docs-to-load by area mapping. Calculates spec readiness score. Returns complete triage package: classification, priority recommendation with reasoning, risk assessment, similar work, assignment suggestions, and docs to load — everything needed to go from raw issue to execution-ready in one call.
  - MCP `analyze_pr_impact` tool: pre-merge impact assessment. Analyzes dependency unblock potential (which issues get unblocked if linked issues close), knowledge risk (bus factor for changed files), coupling analysis from git history (frequently co-changed files that might be missed), hotspot detection (high-churn files getting more churn). Calculates merge readiness score and urgency level. Returns structured impact report with dependency impacts, knowledge risks, coupling warnings, and recommendations.
  - MCP `decompose_issue` tool: AI-powered issue decomposition into subtasks. Generates execution plan based on issue type and complexity: epic/complex issues get full lifecycle (spike → setup → core → error handling → integration → docs), bugs get focused flow (reproduce → fix → verify), standard issues get implement → test. Each subtask includes estimated effort, dependencies, and area classification. Calculates execution order (topological sort), critical path (longest dependency chain), parallelization speedup ratio, and total estimated effort.
- `validate.sh`: Added check for triage.ts
- **What-If Simulation:**
  - MCP `simulate_dependency_change` tool: "What happens if issue #X slips by N days?" Models cascading delay through the dependency graph using BFS with 90% propagation dampening per hop. Shows directly blocked and transitively affected issues with per-issue delay calculations, original vs adjusted completion estimates (from `predict_completion`), and impact severity ratings. Checks critical path impact. Calculates schedule impact: total delay days, worst case, issues delayed, sprint capacity lost. Generates mitigations (parallelize, split bottleneck, prioritize, pause blocked work) and alternative scenarios (partial slip, remove dependency, deliver partial fix). Set `removeIssue=true` to model removing an issue from the dependency chain entirely.
- **Release Intelligence:**
  - MCP `generate_release_notes` tool: automated release notes from merged PRs and closed issues. Parses git merge commits and fetches PR details via gh CLI. Classifies each PR by type (feature/fix/refactor/breaking/docs/chore) from conventional commit prefix and labels. Extracts area from labels or scope. Groups changes by area. Generates: highlights, breaking changes (prominent), changes by area, issues resolved, contributors, technical notes, and a stakeholder-friendly summary (non-technical language). Returns structured data + full markdown. Configurable date range (defaults to last 7 days).
- **Session Intelligence:**
  - MCP `optimize_session` tool: context-aware session planning that recommends the most impactful work. Gathers board state, velocity, events, insights, dependency graph, workflow health, and anomalies in parallel (7 concurrent queries). Analyzes: stalled active issues, review queue (high leverage — unblocks merging), rework pending, dependency bottlenecks, orphaned blocked issues (quick label fix), stale items (14+ days), and critical anomalies. Scores tasks by urgency (immediate/high/normal/low × 4/3/2/1), type leverage (unblock 1.8×, review 1.5×, fix 1.3×), and impact. Fits tasks into available session time, splits into recommended plan, quick wins, and deferrable. Returns session goal, estimated completion, warnings (critical anomalies, review queue backup, low health score), and natural-language summary. Use at the start of every coding session.
- `validate.sh`: Added checks for whatif.ts, release.ts, session.ts
- **Review Intelligence:**
  - MCP `review_pr` tool: structured PR analysis combining file classification by area, scope alignment check (detects area mixing, infra+feature bundling, oversized PRs), acceptance criteria extraction from linked issues, risk assessment (secret exposure, knowledge risk on changed files, large files, missing tests), quality signals (test presence, type changes, config/infra touches), and verdict recommendation (approve/request_changes/needs_discussion) with specific blockers and suggestions. Returns complete review package for automated or assisted code review.
  - MCP `auto_label` tool: automatic issue classification from content analysis. Scans title and body for keyword patterns to suggest type (bug/feature/spike/epic/chore), area (frontend/backend/contracts/infra), priority (critical/high), risk level (high/medium), and spec readiness (checks for Acceptance Criteria and Non-goals sections). Each suggestion includes confidence score (0-1) and reasoning. Labels are suggestions only — never applied automatically. Use for backlog grooming or as a classification assistant.
- **Context Recovery:**
  - MCP `get_session_history` tool: cross-session event history for an issue. Reconstructs: workflow transitions, decisions made, outcomes recorded, approximate session count (clusters of events within 4 hours), and session gaps (periods >1 day between events). Returns chronological timeline of last 50 events. Use to understand what happened with an issue over time and detect stalled work patterns.
  - MCP `recover_context` tool: full context recovery to resume work on an issue. Loads in parallel: current workflow/label/assignee state, previous plans (searches `.claude/plans/` by issue number), linked PR status with review comments, architectural decisions, event timeline, dependency state (blocked-by with resolution status), completion and rework predictions, and all issue comments. Returns a resumption guide with detected mode (START/CONTINUE/REWORK/REVIEW/DONE), prioritized next steps, risk warnings, and context files to load. The "pick up where you left off" single-call context recovery.
- **Batch Operations:**
  - MCP `bulk_triage` tool: triage all untriaged issues in one call. Finds open issues missing type: or area: labels and runs auto_label on each, suggesting classifications. Returns per-issue triage results with suggested type, area, priority, risk, and spec readiness. Labels are suggestions only — not applied automatically. Use for backlog grooming and project cleanup. Configurable max issues (default 20).
  - MCP `bulk_move` tool: move multiple issues between workflow states in one call. Accepts array of issue numbers and target state. Supports dry-run mode to preview changes without applying them. Reports per-issue results: moved, already-in-state, or error. Use for sprint transitions (Ready → Active), cleanup, or batch state corrections.
- **Risk Radar:**
  - MCP `get_risk_radar` tool: comprehensive risk assessment synthesizing ALL intelligence signals into one prioritized view. Gathers 9 data sources in parallel (board, velocity, dependency graph, workflow health, knowledge risk, DORA metrics, team capacity, memory insights, anomaly detection). Evaluates risks across 6 categories: delivery (zero velocity, velocity drops, backlog growth), quality (high rework rate, DORA change failure rate), knowledge (bus factor 1, critical risk files), process (stale items), dependency (cycles, critical bottlenecks not in Active, orphaned blocked), and capacity (decelerating contributors). Each risk includes severity, trend arrow (worsening/stable/improving/new), evidence, affected issues, mitigations with effort/impact ratings, and a 0-100 score. Returns: overall risk score, risk level, prioritized risk list, top 5 risks, per-category health indicators, trend summary, and actionable recommendations. The executive risk dashboard in one call.
- `validate.sh`: Added checks for review-intel.ts, context.ts, batch.ts, risk-radar.ts
...
- **Monte Carlo Simulation:**
  ...
  - Core simulation engine: sequential processing with configurable concurrency (default 1), 0.25-day resolution, 50,000 trial cap, area-filtered cycle times with automatic all-area fallback
...
- `pm-dashboard.sh`: Health Score (0-100) — rework pileup, review bottleneck, backlog bloat, config health
